#!/usr/bin/env bash
# build-all.sh — build Lighthouse flatpak bundles for x86_64 and aarch64.
#
# Usage:
...
#
# Outputs:
#   lighthouse-x86_64.flatpak
#   lighthouse-aarch64.flatpak
...
for arch in "${ARCHES[@]}"; do
    ...
    bundle="lighthouse-${arch}.flatpak"
    echo
    echo "==== Building Lighthouse for ${arch} ===="
    flatpak-builder --arch="$arch" --repo=repo --force-clean \
        "$builddir" build-aux/flatpak/land.rob.lighthouse.json
    echo "==== Bundling ${bundle} ===="
    flatpak build-bundle --arch="$arch" repo "$bundle" land.rob.lighthouse
    ls -lh "$bundle"
done

# ── Optional: install the host-arch bundle ────────────────────────────────
if $INSTALL; then
    bundle="lighthouse-${HOST_ARCH}.flatpak"
    if [[ -f "$bundle" ]]; then
        echo
