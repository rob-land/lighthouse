...
from lighthouse.window import LighthouseWindow


class LighthouseApplication(Adw.Application):
    def __init__(self):
        super().__init__(
    ...
    def do_activate(self):
        ...
        if win is None:
            win = LighthouseWindow(application=self)
        win.present()

    def _show_about(self, *_):
        from lighthouse import APP_NAME, VERSION
...
def main(argv=None):
    configure_logging()
    return LighthouseApplication().run(argv if argv is not None else sys.argv)
